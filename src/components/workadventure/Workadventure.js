...
const Minecraft = () => {
  ...
  useEffect(() => {
    ...
    const getStateAxios = () => {
      axios
        .post(SYSTEM_URL + API.EC2, {
          instance: "workadventure",
          action: "state",
        })
        .then((r) => {
          if (r.data) {
      ...
      axios
        .get(SYSTEM_URL_2 + API.SERVER + "?server=workadventure", {})
        .then((r) => {
          if (r.data) {
            setLogs(r.data);
          }
  ...
  const handleStop = () => {
    axios
      .post(SYSTEM_URL + API.EC2, { instance: "workadventure", action: "stop" })
      .then((r) => {
        if (r.data) {
          setState(r.data);
        }
    ...
    <div style={{ margin: "1rem" }}>
      {!state || !instanceTypes ? (
        ...
        <div className={classes.parrent}>
          ...
          <Typography>
            URL:{" "}
            <a href="https://play.workadventure.h3pa.com/_/global/thecodingmachine.github.io/workadventure-map-starter-kit/map.json">
              play.workadventure.h3pa.com
            </a>
          </Typography>
          <ul>
            <li
          ...
          <ul style={{ border: "1px solid #1e4976" }}>
            {logs.slice(0, 10).map((log) => (
              <li style={{ borderBottom: "1px solid #1e4976", cursor: "auto" }}>
                ...
                {!log.operation.includes("changed") && "Workadventure server"}
              </li>
            ))}
          </ul>
